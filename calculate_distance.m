function distance = calculate_distance(focal_length, principal_point, position, camera_height, horizon_height)
    ...
    ang_obj_principal_axis = atan(double(abs(principal_point(2) - position(2))) / focal_length(2));

    % Angle on the image plane between the principal point and the horizon
    ang_horizon_principal_axis = atan(abs(principal_point(2) - horizon_height) / focal_length(2));
    ...
    ang_obj_horizon = ang_horizon_principal_axis - ang_obj_principal_axis;
    
    % Angle between the vertical line that carries the camera and the
    % connection line of the camera to the buoy
    ang_image_plane_buoy = ang_image_plane_horizon_distance - ang_obj_horizon;
